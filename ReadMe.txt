...
HM_MachineDoc.h、HM_MachineDoc.cpp - 文件
    這些檔案包含您的 CHM_MachineDoc 類別。編輯這些檔案，可加入您的特殊文件類型並實作檔案儲存和載入 (透過 CHM_MachineDoc::Serialize)。
    文件將具有下列字串：
        副檔名：      txt
        檔案類型 ID：        HMMachine.Document
        主框架標題：  HM_Machine
        文件類型名稱：       HM_Machine
        篩選條件名稱：         HM_Machine Files (*.txt)
        檔案新簡短名稱： HM_
        檔案類型完整名稱： HM_Machine.Document

HM_MachineView.h、HM_MachineView.cpp - 文件的檢視表
    這些檔案包含您的 CHM_MachineView 類別。
    CHM_MachineView 物件是用來檢視 CHM_MachineDoc 物件。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 HM_Machine.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。
